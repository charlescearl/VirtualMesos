...
MESOS_INSTALL_HOME = $(DESTDIR)/usr/local/mesos
...
setuptools:
ifneq (.,.)
	mkdir -p ./`dirname $(SETUPTOOLS)`
	cp -f ./$(SETUPTOOLS) ./$(SETUPTOOLS)
endif
...
ifeq ($(OS_NAME),darwin)
	...
	PYTHONPATH=$(SETUPTOOLS):$(MESOS_INSTALL_HOME)/lib/python $(PYTHON) -m easy_install --install-dir $(MESOS_INSTALL_HOME)/lib/python -a ./$(PROTOBUF)/python/dist/*.egg
